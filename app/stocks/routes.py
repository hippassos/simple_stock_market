...
from app.stocks.decorators import (
    numbers_validated,
...
def list_stocks():
    ...
    return {"stocks": [s.__dict__ for s in stocks]}
...
def dividend_yield():
    ...
    return {"dividend_yield": stock.calculate_dividend_yield(price)}
...
def pe_ratio():
    ...
    price = int(request.args.get("price", 0))
    stock = stock_market.get_stock(symbol)
    return {"pe_ratio": stock.calculate_pe_ratio(price)}
